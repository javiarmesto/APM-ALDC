...
3. **Feature-based organization**, not by object type: `src/<Feature>/<SubFeature>/...`. Shared in `Common/` or `Shared/`. Never `Tables/`, `Pages/`, `Codeunits/` folders.
...
5. **Comments only when the why is not obvious**. XML doc (`/// <summary>`) is mandatory only in `public` procedures of codeunits exposed as API.
